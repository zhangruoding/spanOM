...
from .pre_trained_language_model import BERT_input
...
class BiLSTMTaggerModel(nn.Module):
    ...
    @staticmethod
    def get_span_emb(flatted_context_emb, flatted_candidate_starts, flatted_candidate_ends,):
    ...
    def get_dse_unary_scores(self, span_emb):
        input = span_emb
    ...
    def get_srl_scores(self, arg_emb, pred_emb):
    ...
    def forward(self, sent_lengths, input_seq, gold_dse, gold_arg, gold_orl):
       
        seqs_text, words, chars = input_seq#[[sentence]]，(batch,max_sentence_lenth),(batch,max_sentence_lenth,max_word_lenth)
        num_sentences, max_sent_length = words.size()[0], words.size()[1]
       
        #word_embeddings, char_embeddings = self.word_embedding(words), self.get_char_cnn_embeddings(chars)#(batch,max_sentenc_l,300)，(batch,max_sentenc_l,150)
        ...
        if self.config.use_bert:
            bert_embeddings = self.bert_input.forward(seqs_text)#(batch,max_sentence_lenth,768)注意，这里已经去掉了bert的CLS和END位
            #basic_input = torch.cat([basic_input,
             #                        torch.dropout(bert_embeddings, 0, self.training)], dim=2)
        
        orl_input = bert_embeddings#(batch,max_sentence,1218)
        masks = self.init_masks(num_sentences, sent_lengths)#(batch,max_sentence)
        
        lstm_out, _ = self.bilstm(orl_input, masks)#(batch,max_sentence,600)
        ...
        true_pred_dse_starts, predicted_dse_starts_mask = self.ger_predicted_arg_boundary_by_prob_threshold(
            dse_starts_scores, self.config.arg_boundary_prob_threshold, mask=masks)#(batch,max_sentence)
        true_pred_dse_ends, predicted_dse_ends_mask = self.ger_predicted_arg_boundary_by_prob_threshold(
            dse_ends_scores, self.config.arg_boundary_prob_threshold, mask=masks)#(batch,max_sentence)
        ...
        dse_reps = self.dse_reps_drop_0(torch.relu(self.dse_reps_0.forward(lstm_out)))#(batch,max_sentence,300)
        # candidate_starts [num_sentences, max_sent_length * max_arg_width]
        ...
        candidate_dse_span_emb = BiLSTMTaggerModel.get_span_emb(flatted_dse_context_output,
        ...
        #以上完成span的抽取和编码，他是自己做的，现在可以用库完成
        # candidate_dse_span_emb （span_num,400）
        # Get the span ids
        ...
        # Get unary scores and topk of candidate argument spans.
        flatted_candidate_dse_scores = self.get_dse_unary_scores(candidate_dse_span_emb)#（span_num,2）
        if self.config.joint:
        ...
        candidate_span_emb = BiLSTMTaggerModel.get_span_emb(flatted_context_output,flatted_candidate_starts, flatted_candidate_ends)
        ...
        srl_scores = self.get_srl_scores(arg_emb, dse_emb)
